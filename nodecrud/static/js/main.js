...
(function(){
    ...
    // Connection established. 서버에 커넥션 시도.
    socket.on('connected', function (data) {
        console.log(data);

        // Get users connected to mainroom
        socket.emit('getUsersInRoom', {'room':'MainRoom','username':'babo'});
    ...
    socket.on('userNicknameUpdated', function(data) {
        console.log("1, userNicknameUpdated: %s", JSON.stringify(data));
        updateNickname(data);
    ...
    var addMessage = function(msg) {
        getTemplate('js/templates/message.handlebars', function(template) {
			...
			var room_messages = '#room_messages'+'_'+msg.room;
			if(msg.room == 'MainRoom')
